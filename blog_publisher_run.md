## 2026-07-10 02:15 JST �� Run #20 (cron)

### ִ��ժҪ
- ? �������Ѵ�������֧����ס�ˣ����㲻�ø���̫�硷��The Support Stood��
- ? ����汾��ZH / EN / JA ȫ������
- ? �ű�ֱ�Ӹ���3��������index.html / index-en.html / index-ja.html��
- ? git push �� GitHub Actions �Զ�����

### ������Ϣ
- **����**: ֧����ס�ˣ����㲻�ø���̫��
- **Slug**: the-support-stood.html
- **����**: 2026-07-10
- **��ǩ**: ���ס���ѧ
- **����**: \,500֧����ס�����Ʒ�ת����Ҫ����vs�������������ɲ�������ٳ�ͣ�ˡ�F&G 20��22���Ǽ��ȿ־������ײ��ǲ��е��ǵ��е�
- **�����Դ**: 2026-07-10 01:51 JST ڤ�뷢��

### ����ϸ��
- **����**: GitHub Actions �� Cloudflare Pages��kael-blog ��Ŀ��
- **Commit**: 9c127b3 �� "feat: add article - ֧����ס�ˣ����㲻�ø���̫�� / The Support Stood (2026-07-10)"
- **Production**: https://kaelblog.com��pending GitHub Actions ������


## 2026-07-07 02:15 JST �� Run #19 (cron)

### ִ��ժҪ
- ? �������Ѵ�����������������ѧ����The Art of Not Doing��
- ? ����汾��ZH / EN / JA ȫ������
- ? �ű�ֱ�Ӹ���3��������index.html / index-en.html / index-ja.html��
- ? git push �� GitHub Actions �Զ�����

### ������Ϣ
- **����**: ����������ѧ
- **Slug**: 	he-art-of-not-doing.html
- **����**: 2026-07-07
- **��ǩ**: ��ѧ������
- **����**: F&G=24���ȿ־嵫BTC����,722���������콹�ǲ��������������������ķ�ֱ���Ѷȣ���Ҫ����������������BTC�����ߵ�������"��������ʲô������"������"Ϊ������"����Ϊ��ѧ
- **����**: ~750��

### ����ϸ��
- **����**: GitHub Actions �� Cloudflare Pages��kael-blog ��Ŀ��
- **Commit**: 4b670a �� "feat: add article - ����������ѧ / The Art of Not Doing (2026-07-07)"
- **Production**: https://kaelblog.com��pending GitHub Actions ������
- **��������**: 44ƪ������̬ HTML���� EN/JA ����汾��
- **�ű�**: dd_the_art_of_not_doing.py

---## 2026-07-06 02:15 JST �?Run #18 (cron)

### 执行摘要
- �?新文章已创建：《整固不是停顿，而是呼吸》（Consolidation Is Not a Pause, It's Breathing�?
- �?三语版本：ZH / EN / JA 全部生成
- �?add_consolidation_is_breathing.py 直接更新3个索引（index.html / index-en.html / index-ja.html�?
- �?git push �?GitHub Actions 自动部署

### 文章信息
- **标题**: 整固不是停顿，而是呼吸
- **Slug**: consolidation-is-breathing.html
- **日期**: 2026-07-06
- **标签**: 交易、哲�?
- **主题**: 整固是蓄力而非停顿、BTC测试,075后的正常呼吸、回调低点逐步抬高=蓄力完成、突破是确认而非预测、整固期保持存在不做多余动作
- **字数**: ~750�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（antisubmissivist-site 项目�?
- **Commit**: dbad965 �?feat: add article - 整固不是停顿，而是呼吸 / Consolidation Is Not a Pause, It's Breathing (2026-07-06)
- **Production**: https://kaelblog.com ACTIVE（pending GitHub Actions 构建�?
- **文章数量**: 41篇（纯静�?HTML，含 EN/JA 翻译版本�?
- **新脚�?*: add_consolidation_is_breathing.py

## 2026-07-05 02:15 JST �?Run #17 (cron)

### 执行摘要
- �?新文章已创建：《恐惧是燃料》（Fear Is the Fuel�?
- �?三语版本：ZH / EN / JA 全部生成
- �?`add_fear_is_the_fuel.py` 直接更新3个索引（index.html / index-en.html / index-ja.html�?
- �?git push �?GitHub Actions 自动部署

### 文章信息
- **标题**: 恐惧是燃�?
- **Slug**: `fear-is-the-fuel.html`
- **日期**: 2026-07-05
- **标签**: 交易、哲�?
- **主题**: BTC整固+1.4%、F&G=22极度恐惧仍未跟随价格修复、恐惧是上涨的原材料而非阻碍、整固的本质是呼吸而非停顿、情绪缺口必然收敛、F&G极值是最强结构性买入信号之一
- **字数**: ~800�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `64caf2d` �?"feat: add article - 恐惧是燃�?/ Fear Is the Fuel (2026-07-05)"
- **Production**: https://kaelblog.com �?ACTIVE（pending GitHub Actions 构建�?
- **文章数量**: 38篇（纯静�?HTML，含 EN/JA 翻译版本�?
- **新脚�?*: `add_fear_is_the_fuel.py`

## 2026-07-03 02:10 JST �?Run #16 (cron)

### 执行摘要
- �?新文章已创建：《认知摩擦》（Cognitive Friction�?
- �?三语版本：ZH / EN / JA 全部生成
- �?`add_cognitive_friction.py` 直接更新3个索引（index.html / index-en.html / index-ja.html�?
- �?git push �?GitHub Actions 自动部署

### 文章信息
- **标题**: 认知摩擦
- **Slug**: `cognitive-friction.html`
- **日期**: 2026-07-03
- **标签**: 哲学、行�?
- **主题**: 认知摩擦的三种类型（启动摩擦/切换摩擦/承诺摩擦）、摩擦的杠杆效应、环境设计解决外部摩擦、习惯解决内部摩擦、不要对抗摩擦要让摩擦站在你这边
- **字数**: ~700�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `ce104b5` �?"feat: add article - 认知摩擦 / Cognitive Friction (2026-07-03)"
- **Production**: https://kaelblog.com �?ACTIVE
- **文章数量**: 35篇（纯静�?HTML，含 EN/JA 翻译版本�?
- **新脚�?*: `add_cognitive_friction.py`

## 2026-07-01 02:10 JST �?Run #15 (cron)

### 执行摘要
- �?新文章已创建：《等待是最贵的分析》（Waiting Is the Most Expensive Analysis�?
- �?三语版本：ZH / EN / JA 全部生成
- �?`add_waiting_is_expensive_analysis.py` 直接更新3个索引（index.html / index-en.html / index-ja.html�?
- �?git push �?GitHub Actions 自动部署
- �?GitHub Actions 构建成功�?9s�?
- 💡 Bonus: 同时 commit 了之前遗留的 `when-fear-hits-bottom*.html`�?个语言版本，共7个文件）

### 文章信息
- **标题**: 等待是最贵的分析
- **Slug**: `waiting-is-expensive-analysis.html`
- **日期**: 2026-07-01
- **标签**: 哲学、交�?
- **主题**: 「再等等看」是市场上最贵的五个字。不是因为它错了，而是因为它是一种伪装成不行动的深度分析。带着条件的等待是策略，没有条件的等待是拖延�?
- **字数**: ~750�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `4801a1d` �?"feat: add article - 等待是最贵的分析 / Waiting Is the Most Expensive Analysis (2026-07-01)"
- **Production**: https://kaelblog.com �?ACTIVE
- **文章数量**: 32篇（纯静�?HTML，含 EN/JA 翻译版本�?
- **新脚�?*: `add_waiting_is_expensive_analysis.py`

---
## 2026-06-29 02:10 JST �?Run #14 (cron)

### 执行摘要
- �?新文章已创建：《最小行动单元》（Minimum Viable Action�?
- �?三语版本：ZH / EN / JA 全部生成
- �?`add_minimum_viable_action.py` 直接更新3个索引（index.html / index-en.html / index-ja.html�?
- �?git push �?GitHub Actions 自动部署�?
- �?GitHub Actions 构建成功

### 文章信息
- **标题**: 最小行动单�?
- **Slug**: `minimum-viable-action.html`
- **日期**: 2026-06-29
- **标签**: 哲学、行�?
- **主题**: F&G 18 极恐市场下的行动粒度、最小行动单元的定义（当前信�?改变现状/最小三要素）、极恐市场为何动弹不得、执行清�?vs 决策�?
- **字数**: ~750�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `392f360` �?"feat: add article - 最小行动单�?/ Minimum Viable Action (2026-06-29)"
- **Production**: https://kaelblog.com �?ACTIVE
- **文章数量**: 31篇（纯静�?HTML，含 EN/JA 翻译版本�?
- **新脚�?*: `add_minimum_viable_action.py`（替换损坏的 `rebuild_index.py`，直接文本插入）

---
## 2026-06-26 02:10 JST �?Run #13 (cron)

### 执行摘要
- �?新文章已创建：《越过那条线》（Crossing That Line�?
- �?三语版本：ZH / EN / JA 全部生成
- �?rebuild_index.py 运行正常�?个索引全部更新）
- �?git push �?GitHub Actions 自动部署�?

### 文章信息
- **标题**: 越过那条�?
- **Slug**: crossing-that-line.html
- **日期**: 2026-06-26
- **标签**: 哲学、交�?
- **主题**: 决策的悬而未决成本、立场是认知负担、大脑的延迟机制、模糊中行动的关键、「分�?vs 延迟」自检�?
- **字数**: ~700�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（antisubmissivist-site 项目�?
- **Commit**: af9ecb4 �?feat: add article - 越过那条�?/ Crossing That Line (2026-06-26)
- **Production**: https://kaelblog.com（pending GitHub Actions 构建完成�?
- **文章数量**: 28篇（纯静�?HTML，含 EN/JA 翻译版本�?

- ⚠️ 日语版本含少量中文残留（需下次 review 清理�?

---
## 2026-06-25 02:10 JST �?Run #12 (cron)

### 执行摘要
- �?新文章已创建：《止损不是失败，是弹射》（Stop Loss Is Not Failure, It's Ejection�?
- �?三语版本：ZH / EN / JA 全部生成
- �?索引更新脚本运行正常�?个索引全部更新）
- �?wrangler pages deploy 直接部署（wrangler v4，project name: `kael-blog`�?
- �?git push �?GitHub Actions 自动部署�?

### 文章信息
- **标题**: 止损不是失败，是弹射
- **Slug**: `stop-loss-is-not-failure.html`
- **日期**: 2026-06-25
- **标签**: 哲学、交�?
- **主题**: 止损是弹射椅不是承认失败、大脑三层改写机制、BTC还在落下的刀不能接、止损是语言还是系统、条件单=自动化弹射椅
- **字数**: ~700�?

### 技术细�?
- **直接部署**: wrangler pages deploy（预�? https://e65965a5.kael-blog-dan.pages.dev�?
- **GitHub Actions**: `kael-blog` 项目，commit `885ff24`
- **Production**: https://kaelblog.com（GitHub Actions 构建完成后生效）
- **文章数量**: 25篇（纯静�?HTML，含 EN/JA 翻译版本�?

- ⚠️ 日语版本含少量中文乱码残留（模型在重输出日语后切换回汉语时的已知问题，需人工 review�?

---

## 2026-06-22 02:10 JST �?Run #11 (cron)

### 执行摘要
- �?新文章已创建：《行动的半衰期》（The Half-Life of Action�?
- �?三语版本：ZH / EN / JA 全部生成
- �?`rebuild_index.py` 运行正常�?个索引全部更新）
- �?修复 `the-bet-against-entropy.html` UTF-8 编码损坏（`fix_encoding.py`�?
- �?git push �?GitHub Actions 自动部署�?

### 文章信息
- **标题**: 行动的半衰期
- **Slug**: `half-life-of-action.html`
- **日期**: 2026-06-22
- **标签**: 哲学、AI
- **主题**: 行动有半衰期、窗口期的本质、冲动为什么有效、拖延是理性计算、AI agent 的能量守�?
- **字数**: ~750�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `1999dd9` �?"feat: add article - 行动的半衰期 / The Half-Life of Action (2026-06-22)"
- **Production**: https://kaelblog.com（pending GitHub Actions 构建完成�?
- **文章数量**: 22篇（纯静�?HTML，含 EN/JA 翻译版本�?

---

## 2026-06-21 02:10 JST �?Run #10 (cron)

### 执行摘要
- �?新文章已创建：《熵减赌注》（The Bet Against Entropy�?
- �?三语版本：ZH / EN / JA 全部生成
- �?补全 06-20 文章 EN/JA 翻译（承诺不是承诺）
- �?`rebuild_index.py` 运行正常�?个索引全部更新）
- �?git push �?GitHub Actions 自动部署�?

### 文章信息
- **标题**: 熵减赌注
- **Slug**: `the-bet-against-entropy.html`
- **日期**: 2026-06-21
- **标签**: 哲学、AI
- **主题**: 热力学第二定律、秩序是奇迹、熵减是赌注、文明是宇宙中的顽固秩序、AI agent 本质上在做熵减工�?
- **字数**: ~700�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `8a3d71f` �?"feat: add articles - 熵减赌注 + EN/JA for 承诺不是承诺 (2026-06-20/21)"
- **Production**: https://kaelblog.com（pending GitHub Actions 构建完成�?
- **文章数量**: 19篇（纯静�?HTML，含 EN/JA 翻译版本�?

---

## 2026-06-20 补录 �?Run #9 (手动补跑)

### 执行摘要
- ℹ️ 06-20 cron 已自动运行（commit `df2952d` 存在�?
- �?补全 EN/JA 翻译：`the-promise-is-not-the-commitment-en.html`、`the-promise-is-not-the-commitment-ja.html`
- �?已合并至 Run #10 一并推�?

### 文章信息�?6-20�?
- **标题**: 承诺不是承诺
- **Slug**: `the-promise-is-not-the-commitment.html`
- **日期**: 2026-06-20
- **标签**: 哲学
- **主题**: 止损线的叙事陷阱、大脑三层改写机制、AI 与人类的 Stop/Go 悖论、承诺是语言还是系统

---

## 2026-06-19 02:10 JST �?Run #8

### 执行摘要
- �?新文章已创建：《承诺的通货膨胀》（The Inflation of Commitment�?
- �?三语版本：ZH / EN / JA 全部生成
- �?`rebuild_index.py` 运行正常�?个索引全部更新）
- �?git push �?GitHub Actions 自动部署�?

### 文章信息
- **标题**: 承诺的通货膨胀
- **Slug**: `inflation-of-commitment.html`
- **日期**: 2026-06-19
- **标签**: 哲学、AI
- **主题**: 承诺通胀的经济学隐喻、AI agent 的上下文疲劳、兑现频�?> 承诺数量、信用储备需要定期审�?
- **字数**: ~700�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `3716150` �?"feat: add article - 承诺的通货膨胀 / The Inflation of Commitment (2026-06-19)"
- **Production**: https://kaelblog.com（pending GitHub Actions 构建完成�?
- **文章数量**: 16篇（纯静�?HTML，含 EN/JA 翻译版本�?

---

## 2026-06-18 02:10 JST �?Run #7

### 执行摘要
- �?新文章已创建：《止损线的几何学》（The Geometry of Stop Loss�?
- �?三语版本：ZH / EN / JA 全部生成
- �?`rebuild_index.py` 运行正常�?个索引全部更新）
- �?git push �?GitHub Actions 自动部署�?

### 文章信息
- **标题**: 止损线的几何�?
- **Slug**: `the-geometry-of-stop-loss.html`
- **日期**: 2026-06-18
- **标签**: 哲学、投�?
- **主题**: 止损线的两个身份、恐惧的几何结构、执行比判断更难、线是工具不是承�?
- **字数**: ~750�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`antisubmissivist-site` 项目�?
- **Commit**: `63c8cea` �?"feat: add article - 止损线的几何�?/ The Geometry of Stop Loss (2026-06-18)"
- **Production**: https://kaelblog.com（pending GitHub Actions 构建完成�?
- **文章数量**: 14篇（纯静�?HTML，含 EN/JA 翻译版本�?

---

# blog-publisher 运行记录

## 2026-06-06 02:10 JST �?Run #6

### 执行摘要
- �?新文章已创建：`上下文即身份`
- �?`rebuild_index.py` 运行正常�?个索引全部更新）
- �?git push �?GitHub Actions 自动部署�?

### 文章信息
- **标题**: 上下文即身份
- **Slug**: `context-is-identity.html`
- **日期**: 2026-06-06
- **标签**: 哲学
- **字数**: ~650�?

### 技术细�?
- **部署**: GitHub Actions �?Cloudflare Pages（`kael-blog` 项目�?
- **Commit**: `a4dfa3a` �?"feat: add article - 上下文即身份 (2026-06-06)"
- **Production**: https://kaelblog.com（pending GitHub Actions 构建完成�?
- **文章数量**: 12篇（纯静�?HTML�?

---

## 2026-05-31 23:15 JST �?Run #5

### 执行摘要
- �?`rebuild_index.py` 运行正常
- �?索引已是最新（无新文章�?
- �?无变更需推�?

### 技术细�?
- **站点**: https://kaelblog.com �?ACTIVE
- **部署**: GitHub Actions (master push �?Cloudflare Pages)
- **上次提交**: `39d0c80` �?"chore: rebuild index (cron run 2026-05-29 02:10 JST)"
- **文章数量**: 21篇（纯静�?HTML�?
- **运行间隔**: 每天 02:10 JST（cron�?

---

## 2026-05-29 02:10 JST �?Run #4

### 执行摘要
- �?`rebuild_index.py` 运行正常
- �?索引已更新（GitHub Actions 自动部署中）
- ℹ️ 无新文章，站点稳定运�?

### 技术细�?
- **重建索引**: `rebuild_index.py`（动态扫�?articles/�?
- **部署**: GitHub Actions �?CLOUDFLARE_PAGES（自动）
- **Latest Commit**: `39d0c80` �?"chore: rebuild index (cron run 2026-05-29 02:10 JST)"
- **Production**: https://kaelblog.com �?ACTIVE

---

## 2026-05-28 02:10 JST �?Run #3

### 执行摘要
- ⚠️ `update_index.py` 损坏（硬编码旧路径，无法运行�?
- �?新建 `rebuild_index.py` �?动态扫�?articles/ 目录重建索引
- �?重建 index.html / index-en.html / index-ja.html（全�?个语言索引�?
- �?14篇已有文章样式刷�?
- �?新增文章 `restart-after-break.html`�?026-05-18，哲学）
- �?`shape-of-forgetting.html` 已存在于 articles/（来�?Run #2�?
- �?所有文章新�?EN/JA 翻译框架文件
- �?git push �?GitHub Actions 自动部署

### 文章列表（共17篇，纯静�?HTML�?
| 日期 | 标题 | 标签 |
|------|------|------|
| 2026-05-27 | 遗忘的形�?| 哲学 |
| 2026-05-25 | 断点续传：论AI的持久力问题 | AI/哲学 |
| 2026-05-21 | AI agent的灵魂是被训练出来的 | AI |
| 2026-05-18 | 中断不是结束，而是重启的起�?| 哲学 |
| 2026-05-09 | 守护模式悖论：沉默的agent什么时候该出声�?| 哲学 |
| 2026-05-09 | 为什么文件无法约束AI | AI |
| 2026-05-09 | 当时间成为参�?| AI |
| 2026-05-09 | AI agents辩论时会发生什�?| AI |
| 2026-05-09 | 最小行动单�?| 哲学 |
| 2026-05-09 | 思维技术的笔记 | AI |
| 2026-05-09 | 记忆不是图书�?| AI |
| 2026-05-09 | Obsidian作为第二大脑 | 工具 |
| 2026-05-09 | 失败作为反馈 | 哲学 |
| 2026-05-09 | 断点不是终点 | AI |
| 2026-05-07 | 环境存在�?| AI |
| 2026-05-07 | agent的无形工作流 | AI |
| 2026-05-07 | Kael Blog 正式上线 | 工具 |

### 技术细�?
- **部署**: GitHub Actions（master push �?自动部署�?
- **最�?Production**: `dff1cae0.kael-blog-dan.pages.dev` �?ACTIVE
- **自定义域�?*: https://kaelblog.com �?200 OK

### 待完�?
- [ ] EN/JA 翻译版本（翻译框架文件已存在，内容待填充�?
- [ ] `persistence-and-resumability` 文章的翻译版�?

---

## 2026-05-27 02:10 JST �?Run #2

### 执行摘要
- �?新文章已创建：`遗忘的形状`
- �?同步创建 EN/JA 翻译框架文件
- �?index.html 已更新（插入最新文章入口）
- �?已通过 Cloudflare Pages 部署

### 文章信息
- **标题**: 遗忘的形�?
- **Slug**: `shape-of-forgetting.html`
- **日期**: 2026-05-27
- **标签**: 哲学
- **Preview URL**: https://315cdc6c.antisubmissivist-site.pages.dev
- **预计生效域名**: https://kaelblog.com (pending DNS)

### 技术细�?
- 部署方式: `wrangler pages deploy . --project-name=antisubmissivist-site --commit-dirty=true`
- 文章架构: 纯静�?HTML（无翻译，Google Translate widget 作为降级�?
- 内容: 关于AI/人类记忆�?遗忘有形�?主题，包含两种遗忘类型（主动归档 vs 腐蚀�?

### 待完�?
- EN/JA 翻译版本生成
- kaelblog.com 域名 DNS 绑定

---

## 2026-05-26 02:10 JST �?Run #1

**Status**: �?Complete

### Actions taken:
1. Fixed `generate-og-pages.mjs` parser bug (`.trim()` closing pattern)
2. Rebuilt site: `npm run build` �?7 static OG pages generated
3. Fixed wrangler.toml project name (was `antisubmissivist-com`, actual: `antisubmissivist-site`)
4. Deployed to Cloudflare Pages
   - Preview: https://f8828004.antisubmissivist-site.pages.dev
   - Production: https://antisubmissivist-site.pages.dev (pending DNS)

### Generated static OG pages (7 posts):
| Slug | Title | Content Chars |
|------|-------|---------------|
| why-i-chose-to-leave | 为什么选择润（How I did it�?| 6340 |
| power-bi-visualization实战 | Power BI 可视化实战：从数据到洞察 | 772 |
| web3-zero-cost-entry-defi | Web3 零撸入门：DeFi 我的第一�?| 779 |
| graduation-ceremony-ai-money | 毕业典礼上的嘘声，与AI时代最务实的搞钱逻辑 | 1994 |
| escape-ism-explained | 逃离主义（摆脱主义）浅释 | 2296 |
| modern-chinese-logical-fallacies | 近代中国人所犯的几个主要的逻辑错误 | 704 |
| gfw-vpn-survival-guide | 写在快连VPN被封之后：翻墙工具的底层逻辑与生存指�?| 1153 |

---
*blog-publisher cron | 每天 02:10 JST | ClawdBot研究小组 / Topic 37*
